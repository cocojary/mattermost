...
const GlobalThreads = () => {
    ...
    const unreadThreadIdsSelector = useSelector((state: GlobalState) => getUnreadThreadOrderInCurrentTeam(state), shallowEqual);

    // CUSTOM: Delay disappearing of unread items for 5 seconds to improve UX
    const [unreadThreadIds, setUnreadThreadIds] = useState(unreadThreadIdsSelector);
    const prevTeamId = useRef(currentTeamId);

    useEffect(() => {
        if (prevTeamId.current !== currentTeamId) {
            setUnreadThreadIds(unreadThreadIdsSelector);
            prevTeamId.current = currentTeamId;
            return;
        }

        if (unreadThreadIdsSelector.length < unreadThreadIds.length) {
        ...
        setUnreadThreadIds(unreadThreadIdsSelector);
    }, [unreadThreadIdsSelector, currentTeamId, unreadThreadIds.length]);
